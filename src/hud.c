...
MenuAction Hud_DrawMenu(int w, int h) {
    ...
    float bw = 320, bh = 60, gap = 20, bx = (w - bw) / 2.0f, by = h * 0.46f;
    MenuAction a = MENU_NONE;
    if (Button((Rectangle){bx, by + 0 * (bh + gap), bw, bh}, "New Game")) a = MENU_NEW_GAME;
    if (Button((Rectangle){bx, by + 1 * (bh + gap), bw, bh}, "Settings")) a = MENU_SETTINGS;
    if (Button((Rectangle){bx, by + 2 * (bh + gap), bw, bh}, "Exit"))     a = MENU_EXIT;
    return a;
}
